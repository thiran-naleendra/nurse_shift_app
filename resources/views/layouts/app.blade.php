        ...
        <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
                   ...
                   class="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-slate-500">
                            {{ auth()->user()->role === 'admin' ? 'Master admin panel' : 'Manage your own shifts and leave' }}
                        </p>
                    </div>
                </a>
                ...
                <div class="hidden lg:flex items-center gap-2">
                    @if(auth()->user()->role === 'admin')
                        <a href="{{ route('admin.dashboard') }}"
                           class="rounded-xl px-4 py-2 text-sm font-medium {{ request()->routeIs('admin.dashboard') ? 'bg-blue-600 text-white' : 'text-slate-700 hover:bg-slate-100' }}">
                            Admin Dashboard
                        </a>

                        <a href="{{ route('admin.users') }}"
                           class="rounded-xl px-4 py-2 text-sm font-medium {{ request()->routeIs('admin.users.*') ? 'bg-blue-600 text-white' : 'text-slate-700 hover:bg-slate-100' }}">
                            Users
                        </a>
                    @else
            ...
            <div id="mobileMenu" class="hidden border-t border-slate-200 py-4 lg:hidden">
                <div class="mb-4 rounded-2xl border border-slate-200 bg-slate-50 p-4">
                    <div class="flex items-center gap-3">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-slate-500">
                                {{ auth()->user()->role === 'admin' ? 'Developer · Owner · Admin' : 'Developer & Owner' }}
                            </p>
                        </div>
                    </div>
                </div>
                ...
                <div class="flex flex-col gap-2">
                    @if(auth()->user()->role === 'admin')
                        <a href="{{ route('admin.dashboard') }}"
                           class="rounded-xl px-4 py-3 text-sm font-medium {{ request()->routeIs('admin.dashboard') ? 'bg-blue-600 text-white' : 'text-slate-700 hover:bg-slate-100' }}">
                            Admin Dashboard
                        </a>

                        <a href="{{ route('admin.users') }}"
                           class="rounded-xl px-4 py-3 text-sm font-medium {{ request()->routeIs('admin.users.*') ? 'bg-blue-600 text-white' : 'text-slate-700 hover:bg-slate-100' }}">
                            Users
                        </a>
                    @else
                        <a href="{{ route('dashboard') }}"
